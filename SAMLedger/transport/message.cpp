...
#if ZS_CLIENT
	size += sizeof(leader_id);
	size += sizeof(do_or_pending);
	size += sizeof(special_id);
#endif
...
#if ZS_CLIENT
	COPY_VAL(leader_id, buf, ptr);
	COPY_VAL(do_or_pending, buf, ptr);
	COPY_VAL(special_id, buf, ptr);
#endif
...
#if ZS_CLIENT
	COPY_BUF(buf, leader_id, ptr);
	COPY_BUF(buf, do_or_pending, ptr);
	COPY_BUF(buf, special_id, ptr);
#endif
...
#if ZS
// std::cout << "ZS: getsize begin: batchid"<<endl;
//  std::cout << "ZS: getsize 1 end: batchid"<<batch_id<<" txnid:"<<txn_id<<"getsize: size:" <<size << " sizeof StateShardSize:"<<size_EachStateShard.size()
// 	<<"size of StateShard:"<<StateShard.size()<< std::endl;
	size += sizeof(flag_shard);
// std::cout << "flag_shard:"<< flag_shard <<" after flag_shard size:"<<size << std::endl;
	if(flag_shard!=0){
		
		size += sizeof(shardDag_id);
		size += sizeof(avg_workload);
		//  std::cout << "shardDag_id:"<< shardDag_id <<" after shardDag_id size:"<<size << std::endl;
		size_ShardDAG=ShardDAG.size();

		size_StateShard=StateShard.size();

		size_StateValue=StateValue.size();

		size += sizeof(size_ShardDAG);
				//  std::cout << "size_ShardDAG:"<< size_ShardDAG <<" after size_ShardDAG size:"<<size << std::endl;
		size += sizeof(size_StateShard);
				//  std::cout << "size_StateShard:"<< size_StateShard <<" after size_StateShard size:"<<size << std::endl;
		size += sizeof(size_StateValue);
				//  std::cout << "size_StateValue:"<< size_StateValue <<" after size_StateValue size:"<<size << std::endl;

		size += 3 * sizeof(uint64_t) * size_ShardDAG;
				//  std::cout <<" after ShardDAG size:"<<size << std::endl;
		size += sizeof(uint64_t) * g_shard_cnt;
				//  std::cout <<" after StateShardSize size:"<<size << std::endl;

	for(uint64_t i=0;i<g_shard_cnt;i++){
		if(size_EachStateShard[i]!=0){
			size +=  sizeof(uint64_t) * size_EachStateShard[i];
			// std::cout << "Shard: " << i << ", StateShardsize: " << StateShard[i].size() <<" size_EachStateShard[i]="<<size_EachStateShard[i]<< std::endl;
		}
	}


	// std::cout <<" after StateShard size:"<<size << std::endl;
		...
		// size += 2 * sizeof(uint64_t) * size_StateShard;
		// 		 std::cout <<" after StateShard size:"<<size << std::endl;
		size += 2 * sizeof(uint64_t) * size_StateValue;
				//  std::cout <<" after StateValue size:"<<size << std::endl;
	}
	//  std::cout << "ZS: batchid"<<batch_id<<" txnid:"<<txn_id<<"end getsize: size:"<<size << std::endl;
	//   std::cout << "ZS: getsize 2 end: batchid"<<batch_id<<" txnid:"<<txn_id<<"getsize: size:" <<size << " sizeof StateShardSize:"<<size_EachStateShard.size()
	// <<"size of StateShard:"<<StateShard.size()<< std::endl;
	// std::cout << "ZS: getsize end: batchid"<<endl;
#endif
...
// Only primary should create this message
	...
	this->avg_workload=0;
    this->ShardDAG.clear();
    this->StateShard.clear();
    this->StateValue.clear();
	size_EachStateShard.clear();
	for(uint64_t i=0;i<g_shard_cnt;i++)
	{
		size_EachStateShard.push_back(0);
	}
	this->size_ShardDAG=this->ShardDAG.size();
	this->size_StateShard=this->StateShard.size();
	this->size_StateValue=this->StateValue.size();
...
void BatchRequests::release()
{
	...
	shardDag_id=0;
	avg_workload=0;
    std::vector<std::array<uint64_t, 3>>().swap(ShardDAG);
	std::map<uint64_t, std::vector<uint64_t>>().swap(StateShard);
	map <uint64_t,uint64_t>().swap(StateValue);
	for(uint64_t i=0;i<g_shard_cnt;i++)
	{
		size_EachStateShard.push_back(0);
	}
...
#if ZS
// std::cout << "ZS: copy_from_buf begin: batchid"<<batch_id<<" txnid:"<<txn_id<<"getsize: size:"<<ptr << std::endl;
	// std::cout << "ZS: copy_from_buf begin: flag_shard:"<<flag_shard << std::endl;
	COPY_VAL(flag_shard, buf, ptr);
	if(flag_shard!=0){
	COPY_VAL(shardDag_id, buf, ptr);
	
	COPY_VAL(avg_workload, buf, ptr);
	// std::cout << "shardDag_id:"<<shardDag_id <<" avg_workload:"<<avg_workload <<std::endl;
	// // 遍历 ShardDAG，打印每个 std::array<uint64_t, 3>
	// std::cout << "copy_from_buf sizeShard contents:" << std::endl;
	COPY_VAL(size_ShardDAG, buf, ptr);
	COPY_VAL(size_StateShard, buf, ptr);
	COPY_VAL(size_StateValue, buf, ptr);
	// std::cout << "size_ShardDAG:" << size_ShardDAG << ", size_StateShard:" << size_StateShard << ", size_StateValue:" << size_StateValue << ")" << std::endl;
	
    // std::cout << "copy_from_buf ShardDAG contents:" << std::endl;
	for(uint64_t j = 0; j < size_ShardDAG; j++){
		    uint64_t u,v,w;
			COPY_VAL(u, buf, ptr);
			COPY_VAL(v, buf, ptr);
			COPY_VAL(w, buf, ptr);
			ShardDAG.push_back({u, v, w});
			std::cout << "(" << u << ", " << v << ", " << w << ")" << std::endl;
	}
	// std::cout << "copy_from_buf ShardDAG contents end" << std::endl;
	// std::cout << "copy_from_buf StateShard contents:" << std::endl;
	
// std::cout << "copy_from_buf ShardDAG end before sizeof StateShardSize:"<< "shardDag_id=" << shardDag_id<<size_EachStateShard.size()
// 	<<"size of StateShard:"<<StateShard.size() << std::endl;
	...
	for(uint64_t i=0;i<g_shard_cnt;i++)
	{
		COPY_VAL(tempsize, buf, ptr);
		size_EachStateShard.push_back(tempsize);
		// std::cout << "shard: " << i << ", size: " << size_EachStateShard[i] << std::endl;
	}
	// std::cout << "copy_from_buf ShardDAG end after sizeof StateShardSize:"<< "shardDag_id=" << shardDag_id<<size_EachStateShard.size()
	// <<"size of StateShard:"<<StateShard.size() << std::endl;
	...
	for(uint64_t i=0;i<g_shard_cnt;i++){
		if(size_EachStateShard[i]!=0){
			...
			uint64_t x;
			for(uint64_t j=0;j<size_EachStateShard[i];j++){
				
				COPY_VAL(x, buf, ptr);
				vec.push_back(x);
			}
			StateShard[i]=vec;
			std::cout << "copy_from_buf shardDag_id=" << shardDag_id<<" Shard: " << i 
			<< ", StateShard.size(): " << StateShard[i].size() << "size_EachStateShard="<<size_EachStateShard[i]<< std::endl;
		}else{
			// std::cout << "copy_from_buf shardDag_id=" << shardDag_id<<" Shard: " << i 
			// << ", StateShard.size(): " << 0 << "size_EachStateShard="<<size_EachStateShard[i]<< std::endl;
		}
	}
	// 	std::cout << "copy_from_buf ShardDAG end after sizeof StateShard:"<< "shardDag_id=" << shardDag_id<<size_EachStateShard.size()
	// <<"size of StateShard:"<<StateShard.size() << std::endl;



	// for(uint64_t j = 0; j < size_StateShard; j++){
	// 	    uint64_t x,y;
	// 		COPY_VAL(x, buf, ptr);
	// 		COPY_VAL(y, buf, ptr);
	// 		StateShard[x]=y;
	...
	// }
	// std::cout << "copy_from_buf StateValue contents:" << std::endl;
	for(uint64_t j = 0; j < size_StateValue; j++){
		    uint64_t x,y;
			COPY_VAL(x, buf, ptr);
			COPY_VAL(y, buf, ptr);
			StateValue[x]=y;
	...
	// std::cout << "copy_from_buf end" << std::endl;
	}
	// std::cout << "ZS: copy_from_buf end: batchid"<<batch_id<<" txnid:"<<txn_id<<"getsize: size:"<<ptr << std::endl;
	// std::cout << "ZS: copy_from_buf end: batchid"<<endl;
...
#if ZS
// std::cout << "ZS: copy_to_buf begin: batchid"<<endl;
// std::cout << "ZS: copy_to_buf begin: batchid"<<batch_id<<" txnid:"<<txn_id<<"getsize: size:"<<ptr << "sizeof StateShardSize:"<<size_EachStateShard.size()
// 	<<"size of StateShard:"<<StateShard.size()<< std::endl;
// std::cout << "ZS: copy_to_buf begin: flag_shard:"<<flag_shard <<"shardDag_id:"<<shardDag_id<< std::endl;
	COPY_BUF(buf, flag_shard, ptr);
	if(flag_shard!=0){
	// 遍历 ShardDAG，打印每个 std::array<uint64_t, 3>
	COPY_BUF(buf, shardDag_id, ptr);
	COPY_BUF(buf, avg_workload, ptr);
	
	size_ShardDAG=ShardDAG.size();
	size_StateShard=StateShard.size();
	size_StateValue=StateValue.size();
	COPY_BUF(buf, size_ShardDAG, ptr);
	COPY_BUF(buf, size_StateShard, ptr);
	COPY_BUF(buf, size_StateValue, ptr);


    // std::cout << "copy_to_buf ShardDAG contents:" << std::endl;
    for (const auto& arr : ShardDAG) {
		COPY_BUF(buf, arr[0], ptr);
		COPY_BUF(buf, arr[1], ptr);
		COPY_BUF(buf, arr[2], ptr);
        std::cout << "(" << arr[0] << ", " << arr[1] << ", " << arr[2] << ")" << std::endl;
    }
	// std::cout << "copy_to_buf ShardDAG end sizeof StateShardSize:"<<size_EachStateShard.size()
	// <<"size of StateShard:"<<StateShard.size() << std::endl;

	for(uint64_t i=0;i<g_shard_cnt;i++)
	{
		COPY_BUF(buf, size_EachStateShard[i], ptr);
		//  std::cout << "shardDag_id=" << shardDag_id<< "shard: " << i << ", size: " << size_EachStateShard[i] << std::endl;
	}


// std::cout << "copy_to_buf ShardDAG end after sizeof StateShardSize:"<< "shardDag_id=" << shardDag_id<<size_EachStateShard.size()
// 	<<"size of StateShard:"<<StateShard.size() << std::endl;

	for(uint64_t i=0;i<g_shard_cnt;i++){
		if(size_EachStateShard[i]!=0){
			for(uint64_t j=0;j<size_EachStateShard[i];j++){
				COPY_BUF(buf, StateShard[i][j], ptr);
			}
			std::cout << "shardDag_id=" << shardDag_id<<" Shard: " << i 
			<< ", StateShard.size(): " << StateShard[i].size() << "size_EachStateShard="<<size_EachStateShard[i]<< std::endl;
		}else{
			// std::cout << "shardDag_id=" << shardDag_id<<" Shard: " << i 
			// << ", StateShard.size(): " << 0 << "size_EachStateShard="<<size_EachStateShard[i]<< std::endl;
...
// std::cout << "copy_to_buf ShardDAG end after size of StateShard:"<< "shardDag_id=" << shardDag_id<<size_EachStateShard.size()
// 	<<"size of StateShard:"<<StateShard.size() << std::endl;

    // 遍历 StateShard，打印每个键值对
    // std::cout << "\ncopy_to_buf StateShard contents:" << std::endl;
    // for (const auto& pair : StateShard) {
	// 	COPY_BUF(buf, pair.first, ptr);
	// 	COPY_BUF(buf, pair.second, ptr);
        
    // }

    // 遍历 StateValue，打印每个键值对
    // std::cout << "\ncopy_to_buf StateValue contents:" << std::endl;
    for (const auto& pair : StateValue) {
		COPY_BUF(buf, pair.first, ptr);
		COPY_BUF(buf, pair.second, ptr);
...
// std::cout << "ZS: copy_to_buf end: batchid"<<endl;
// std::cout << "ZS: copy_to_buf end: batchid"<<batch_id<<" txnid:"<<txn_id<<"getsize: size:" <<ptr << " sizeof StateShardSize:"<<size_EachStateShard.size()
// 	<<"size of StateShard:"<<StateShard.size()<< std::endl;
#endif
