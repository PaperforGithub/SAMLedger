...
UInt32 g_is_sharding = RING_BFT || SHARPER || NEW_SHARPER;
...
// 初始化客户端发送速率数组
// 这里设置10个不同的发送速率，从10%到100%
double g_client_send_rates_ctrl[CLIENT_NODE_CNT] = {
    10.0,   // 客户端0: 10%速率
    100.0,   // 客户端1: 100%速率
};
#endif
...
double g_random_choose_shard_prob[NODE_CNT/SHARD_SIZE] = {
	25.0,
	25.0,
	25.0,
	25.0,
...
double g_random_choose_no_cross_shard_prob[NODE_CNT/SHARD_SIZE] = {
	25.0,
	25.0,
	25.0,
	25.0,
...
#if SHARPER || RING_BFT || NEW_SHARPER
...
uint64_t g_cross_shard_prob = CROSS_SHARD_PRECENTAGE;
#endif
...
uint64_t get_shard_number(uint64_t i)
{
	...
	{
		int client_number = i - g_node_cnt;
		// cout << "test_v5:get_shard_number: client_number = " << client_number << "result = "<< (client_number / (g_client_node_cnt / g_shard_cnt)) << ", g_client_node_cnt = " << g_client_node_cnt << ", g_shard_cnt = " << g_shard_cnt << "\n";
		return (client_number / (g_client_node_cnt / g_shard_cnt));
	}
	else
		return (i / g_shard_size);
