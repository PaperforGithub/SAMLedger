...
void WorkerThread::process(Message *msg)
{
    ...
    case KEYEX:
        ...
        break;
    case CL_BATCH:
        //cout << "test_v8:get_CL_BATCH, txn_id = " << msg->txn_id << "is_cross_shard = " << msg->is_cross_shard << "\n";
...
//                     BatchRequests *response = g_dag.model_BR;
...
if(msg->rtype==BATCH_REQ){
    BatchRequests *breq = (BatchRequests *)msg;
    // cout <<endl<<"ZS run BR batchid="<<breq->batch_id
    // <<" txid="<<breq->txn_id<<" flag_shard="<<breq->flag_shard
    // <<" shardDag_id="<<breq->shardDag_id<<endl;
    if(breq->flag_shard==1){
        cout << "ZS: DataMigration Step2: shardDag_id="<<breq->shardDag_id
            <<"flag_shard=1 recv at node:"<<g_node_id<<endl;

        char *buf = create_msg_buffer(breq);
        Message *deepMsg = deep_copy_msg(buf, breq);
        BatchRequests *response=(BatchRequests *)deepMsg;
        response->flag_shard=2;
        response->ShardDAG.clear();
        g_dag.getCurrentShardDAG(breq->shardDag_id,response->ShardDAG);
        ...
        cout << "ZS: DataMigration Step2: shardDag_id="<<response->shardDag_id
            <<"flag_shard=2 send at node:"<<g_node_id<<" cnt_b="<< g_dag.cnt_batch<<endl;
        dest.clear();
        ...
        continue;
    }else if(breq->flag_shard==2){
        cout << "ZS: DataMigration Step3: shardDag_id="<<breq->shardDag_id
            <<"flag_shard=2 recv at node:"<<g_node_id<<endl;

        cout << "ZS: DataMigration Step3: shardDag_id="<<breq->shardDag_id
            <<"flag_shard=2 recv at node:"<<g_node_id<<"recv ShardDag"<<endl;

        g_dag.addShardDAG(breq->shardDag_id,breq->ShardDAG);

        // for (const auto& arr : breq->ShardDAG) {
        //         // it->second->addEdge(arr[0],arr[1],arr[2]);
        //         std::cout << "("<<arr[0]<<","<<arr[1]<<","<<arr[2]<< std::endl;
        //     }
        cout << "ZS: DataMigration Step3: shardDag_id="<<breq->shardDag_id
            <<" flag_shard=2 cntPSDAG="<<g_dag.cntPendingShardDAG[breq->shardDag_id]
            <<" g_shard_cnt="<<g_shard_cnt<<endl;

        if(g_dag.cntPendingShardDAG[breq->shardDag_id]==g_shard_cnt){
            ...
            response->flag_shard=3;
            cout << "ZS: DataMigration Step3: shardDag_id="<<breq->shardDag_id
                <<"flag_shard=3 before updateStateShard"<<endl;
            g_dag.updateStateShard(breq->shardDag_id,response->size_EachStateShard,response->StateShard,response->avg_workload);

            std::cout << "ZS: DataMigration Step3 updateShardDAG beforeq shardDag_id: " << breq->shardDag_id<< "resStateShard Size "<<response->StateShard.size()<<endl;
            for(uint64_t i=0;i<g_shard_cnt;i++){
                std::cout << "ZS: DataMigration Step3 updateShardDAG beforeq shardDag_id: " << breq->shardDag_id<< "shard= "<<i<<"size="<<response->size_EachStateShard[i];
                if(response->size_EachStateShard[i]!=0){
                    cout <<" stateshard["<<i<<"].size()="<<response->StateShard[i].size();
                }
                cout <<endl;
            }
            ...
            response->size_StateShard=response->StateShard.size();
            response->ShardDAG.clear();
            response->size_ShardDAG=response->ShardDAG.size();
            cout << "ZS: DataMigration Step3: shardDag_id="<<breq->shardDag_id
                <<"flag_shard=3 size size_ShardDAG="<<response->size_ShardDAG
                <<" response->size_StateShard"<<response->size_StateShard<<endl;
            cout << "ZS: DataMigration Step3: shardDag_id="<<breq->shardDag_id
                <<"flag_shard=3 dest: ";
            vector<uint64_t> dest;
                    for (uint64_t i = 0; i < g_node_cnt; i++)
                    {
                        if(i!=g_node_id && i%g_shard_size==0){
                            dest.push_back(i);
                                                cout << " dest="<<i<<endl;
                        }
                    }
            // cout << "ZS: BQ shardDag_id="<<response->shardDag_id
            //     <<"flag_shard=3 send at node:"<<g_node_id<<" cnt_b="<< g_dag.cnt_batch<<endl;
            msg_queue.enqueue(get_thd_id(), response, dest);
            // cout << "ZS: BQ shardDag_id="<<response->shardDag_id
            //     <<"flag_shard=3 send after node:"<<g_node_id<<" cnt_b="<< g_dag.cnt_batch<<endl;
            std::cout << "ZS: DataMigration Step3 updateShardDAG afterq shardDag_id: " << breq->shardDag_id<< "resStateShard Size "<<response->StateShard.size()<<endl;
            for(uint64_t i=0;i<g_shard_cnt;i++){
                std::cout << "ZS: DataMigration Step3 updateShardDAG afterq shardDag_id: " << breq->shardDag_id<< "shard= "<<i<<"size="<<response->size_EachStateShard[i];
                if(response->size_EachStateShard[i]!=0){
                    cout <<" stateshard["<<i<<"].size()="<<response->StateShard[i].size();
                }
                cout <<endl;
            }
            cout << "ZS: DataMigration Step3: shardDag_id="<<response->shardDag_id
                <<"flag_shard=3 send at node:"<<g_node_id<<endl;
            dest.clear();

            g_dag.updatePendingBatchShardDag_id(breq->shardDag_id);
            auto it=g_dag.PendingBatch.find(breq->shardDag_id);
            if (it!=g_dag.PendingBatch.end()){
                for (ClientQueryBatch* tempclbtch : it->second) {
                    char *buf = create_msg_buffer(tempclbtch);
                    Message *deepMsg = deep_copy_msg(buf, tempclbtch);
                    ClientQueryBatch *deepclbtch = (ClientQueryBatch *)deepMsg;
                    deepclbtch->do_or_pending=tempclbtch->do_or_pending=1;
            ...
            g_dag.releasePendingBatchShardDag_id(breq->shardDag_id);
        }
        ...
        Message::release_message(msg);
        INC_STATS(get_thd_id(), worker_release_msg_time, get_sys_clock() - ready_starttime);
        // cout <<endl<< "ZS run release flag=2"<<endl;
        continue;

    }else if(breq->flag_shard==3){

        cout << "ZS: DataMigration Step4: shardDag_id="<<breq->shardDag_id
            <<" flag_shard=3 recv StateShard"<<endl;
        std::cout << "ZS: DataMigration Step4 updateShardDAG afterq shardDag_id=" << breq->shardDag_id<< " resStateShard Size "<<breq->StateShard.size()<<endl;
            for(uint64_t i=0;i<g_shard_cnt;i++){
                std::cout << "ZS: DataMigration Step3 updateShardDAG afterq shardDag_id=" << breq->shardDag_id<< "shard= "<<i<<"size="<<breq->size_EachStateShard[i];
                if(breq->size_EachStateShard[i]!=0){
                    cout <<" stateshard["<<i<<"].size()="<<breq->StateShard[i].size();
                }
                cout <<endl;
            }
        g_dag.updateStateShard(breq->shardDag_id,breq->size_EachStateShard,breq->StateShard,breq->avg_workload);
                    ...
                    if(deepclbtch->leader_id==g_node_id){
                        ...
                        // cout<<" process in this node"<<endl;
                            ...
                            if(deepclbtch->leader_id==0){
                                cout<<endl<<"ZS batchid="<<deepclbtch->batch_id<<" work thread after updateClientQueryBatch txid="<<deepclbtch->txn_id
                                    <<" DoP="<<deepclbtch->do_or_pending
                                    <<" newiscross="<<deepclbtch->is_cross_shard <<"leader_id="<<deepclbtch->leader_id;
                                    cout << "g_dag.remote_shard_id="<<g_node_id;
                                ...
                                deepclbtch->leader_id=g_node_id;
                                deepclbtch->is_cross_shard=false;
                                for (uint32_t i = 0; i < g_shard_cnt; ++i) {
                                    deepclbtch->involved_shards[i]=false;
                                }
                                deepclbtch->involved_shards[g_node_id/g_shard_size]=true;
                                work_queue.enqueue(get_thd_id(), deepclbtch, false);
                            }else
                        #endif
                            {
        ...
        Message::release_message(msg);
        INC_STATS(get_thd_id(), worker_release_msg_time, get_sys_clock() - ready_starttime);
        // cout <<endl<< "ZS run release flag=3"<<endl;
        continue;
    }
...
#endif
    //cout << "test_v8:send_execute_msg, txn_id = " << tmsg->txn_id << "is_cross_shard = " << tmsg->is_cross_shard  << "expectedExecuteCount = " << get_expectedExecuteCount() << "expectedExecuteNetId = " << get_expectedExecuteNetId() << "\n";
...
RC WorkerThread::process_execute_msg(Message *msg)
{
    ...
    //cout << "test_v8:process_execute_msg, txn_id = " << emsg->txn_id << "\n";
    //cout << "test_v7:process_execute_msg: batch_id = " << emsg->batch_id << "txn_id = " << emsg->txn_id << "is_cross_shard = " << emsg->is_cross_shard << "net_id = " << emsg->net_id <<"\n";
    ...
    //re_execute all txn
        ...
        if(is_re_execute){
            ...
            #if NEW_SHARPER
            if(emsg->is_cross_shard){
                INC_STATS(get_thd_id(), cross_shard_txn_cnt, get_batch_size());
            }
            else{
                INC_STATS(get_thd_id(), no_cross_shard_txn_cnt, get_batch_size());
            }
            #endif
            ...
            else
            {
                ...
                #if NEW_SHARPER
                if(emsg->is_cross_shard){
                    INC_STATS(get_thd_id(), cross_shard_txn_cnt, valid_count);
                }
                else{
                    INC_STATS(get_thd_id(), no_cross_shard_txn_cnt, valid_count);
                }
                #endif
    ...
    #if NEW_SHARPER
        crsp->is_cross_shard = emsg->is_cross_shard;
    #endif
    
    vector<uint64_t> dest;
    dest.push_back(txn_man->client_id);
    //cout << "test_v7:before_enqueue:crsp->is_cross_shard = " << crsp->is_cross_shard << "\n";
...
void WorkerThread::create_and_send_batchreq(ClientQueryBatch *msg, uint64_t tid)
{
    ...
    for (uint64_t i = 0; i < get_batch_size(); i++)
    {
        ...
        //cout << "test_v8:create_and_send_batchreq, txn_id = " << txn_id << "\n";
