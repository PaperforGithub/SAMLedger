...
#if NEW_SHARPER
    //cross_shard_txn_cnt += stats->cross_shard_txn_cnt;
    no_cross_shard_txn_cnt += stats->no_cross_shard_txn_cnt;
#endif
...
void Stats::print(bool prog)
{
    ...
    double tput = 0, interval_tput = 0, interval_c_tput = 0;
#if ISEOV && CHECK_CONFILICT
    double valid_tput = 0;
#endif
#if NEW_SHARPER
    double cross_shard_tput = 0;
    double no_cross_shard_tput = 0;
#endif
...
#endif
#if NEW_SHARPER
        cross_shard_tput = totals->cross_shard_txn_cnt / (totals->total_runtime / BILLION);   
        no_cross_shard_tput = totals->no_cross_shard_txn_cnt / (totals->total_runtime / BILLION);
#endif
...
#endif    
#if NEW_SHARPER
    fprintf(outf, "cross_shard_tput         =%f\tcross_shard_txn_cnt=%ld\n", cross_shard_tput, totals->cross_shard_txn_cnt);
    fprintf(outf, "no_cross_shard_tput         =%f\tno_cross_shard_txn_cnt=%ld\n", no_cross_shard_tput, totals->no_cross_shard_txn_cnt);
#endif
    fflush(outf);
    if (!prog)
    {
...
#endif
#if NEW_SHARPER
    fprintf(outf, "\ncross_shard_tput=%f\nno_cross_shard_tput=%f\n", cross_shard_tput, no_cross_shard_tput);
#endif
